...
extern "C" int cel_GenericErrorSpace_CurrentCode() { return errno; }

extern "C" int cel_SystemErrorSpace_CurrentCode() {
#ifndef _WIN32
...
static int _cel_CanonicalErrorSpace_Message(CEL_NONNULL(const cel_ErrorSpace*)
  ...
  const char* desc = cel_StatusCode_Message(static_cast<cel_StatusCode>(code));
  if (desc == cel_nullptr) {
    buf[0] = '\0';
    return 0;
  }
