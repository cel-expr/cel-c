...
class TypeFromProtoTest : public ::testing::Test {
 public:
  void SetUp() override {
 ...
 protected:
  ...
  cel_expr_Type* ParseTextProto(absl::string_view text) {
    cel::expr::Type proto;
    ...
    return ABSL_DIE_IF_NULL(cel_expr_Type_parse(
...
TEST_F(TypeFromProtoTest, Dyn) {
  const cel_Type* type =
      cel_Type_FromProto(ParseTextProto(R"pb(dyn: {})pb"), arena(), status());
...
TEST_F(TypeFromProtoTest, Null) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, Double) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, String) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, Bytes) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, BoolWrapper) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, IntWrapper) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, UintWrapper) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, DoubleWrapper) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, StringWrapper) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, BytesWrapper) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, Duration) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, Timestamp) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, Map) {
  const cel_Type* type =
      cel_Type_FromProto(ParseTextProto(R"pb(map_type: {
...
TEST_F(TypeFromProtoTest, Function) {
  const cel_Type* type =
      cel_Type_FromProto(ParseTextProto(R"pb(function: {
...
TEST_F(TypeFromProtoTest, Struct) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, TypeParam) {
  const cel_Type* type = cel_Type_FromProto(
...
TEST_F(TypeFromProtoTest, Error) {
  const cel_Type* type =
      cel_Type_FromProto(ParseTextProto(R"pb(error: {})pb"), arena(), status());
  ASSERT_THAT(type, NotNull());
  EXPECT_TRUE(cel_Status_Ok(status()));
...
TEST_F(TypeFromProtoTest, Opaque) {
  const cel_Type* type = cel_Type_FromProto(
