...
cel_HashState cel_HashState_Initialize() {
...
cel_HashState cel_HashState_CombineN(cel_HashState state,
  ...
  if (size > 16) {
    if (CEL_UNLIKELY(size > 1024)) {
      do {
        state.val = _cel_HashMix(state.val, _cel_LowLevelHashN(data, 1024));
        size -= 1024;
    ...
    uint64_t lo;
    uint64_t hi;
    _cel_HashRead9To16(data, size, &lo, &hi);
    lo = _cel_rotr(lo, 53);
    ...
    v = _cel_HashRead4To8(data, size);
  } else if (size > 0) {
    v = _cel_HashRead1To3(data, size);
  } else {
    return state;
  }
...
cel_HashState cel_HashState_Combine64(cel_HashState state, uint64_t data) {
  return ((cel_HashState){_cel_HashMix(state.val, data)});
}

cel_HashState cel_HashState_CombineLD(cel_HashState state, long double value) {
