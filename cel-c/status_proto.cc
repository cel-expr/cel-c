...
static bool _cel_Status_StrDup(cel_StringView in, CEL_NONNULL(upb_Arena*) arena,
  ...
  char* copied =
      reinterpret_cast<char*>(upb_Arena_Malloc(arena, cel_StringView_Size(in)));
...
extern "C" bool cel_Status_ToProto(CEL_NONNULL(const cel_Status*) in,
  ...
  google_rpc_Status_set_code(
      out, cel_StatusCode_ToProto(cel_Status_CanonicalCode(in)));
...
extern "C" bool cel_Status_FromProto(CEL_NONNULL(cel_Status*) out,
                                     CEL_NONNULL(const google_rpc_Status*) in) {
  CEL_ASSERT_NOT_NULL(in);
  CEL_ASSERT_NOT_NULL(out);

  google_rpc_Code in_code =
      static_cast<google_rpc_Code>(google_rpc_Status_code(in));
