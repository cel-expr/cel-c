...
extern "C" cel_Decl* cel_nullable cel_Decl_FromProto(
  ...
  const cel_expr_Decl_decl_kind_oneofcases decl_kind =
      cel_expr_Decl_decl_kind_case(in);
  switch (decl_kind) {
    case cel_expr_Decl_decl_kind_ident: {
      cel_StringView out_name;
      if (CEL_UNLIKELY(!cel_Arena_StrDup(arena, &out_name,
                                         cel_expr_Decl_name(in)))) {
      ...
      const cel_expr_Decl_IdentDecl* in_ident =
          cel_expr_Decl_ident(in);
      if (cel_expr_Decl_IdentDecl_has_type(in_ident)) {
        const cel_Type* out_type = cel_Type_FromProto(
            cel_expr_Decl_IdentDecl_type(in_ident), arena, status);
        if (CEL_UNLIKELY(out_type == cel_nullptr)) {
          return cel_nullptr;
        }
        cel_IdentDecl_SetType(out, out_type);
      }
      if (cel_expr_Decl_IdentDecl_has_value(in_ident)) {
        if (CEL_UNLIKELY(!cel_Constant_FromProto(
                cel_IdentDecl_MutableValue(out),
                cel_expr_Decl_IdentDecl_value(in_ident), arena,
                status))) {
          return cel_nullptr;
        }
      }
      ...
      if (CEL_UNLIKELY(!cel_Arena_StrDup(
              arena, &out_doc, cel_expr_Decl_IdentDecl_doc(in_ident)))) {
        cel_OutOfMemoryStatus(status);
        return cel_nullptr;
      }
    ...
    case cel_expr_Decl_decl_kind_function: {
      cel_StringView out_name;
      if (CEL_UNLIKELY(!cel_Arena_StrDup(arena, &out_name,
                                         cel_expr_Decl_name(in)))) {
        cel_OutOfMemoryStatus(status);
      ...
      const cel_expr_Decl_FunctionDecl* in_function =
          cel_expr_Decl_function(in);
      size_t in_overloads_len;
      const cel_expr_Decl_FunctionDecl_Overload* const*
          in_overloads_ptr = cel_expr_Decl_FunctionDecl_overloads(
              in_function, &in_overloads_len);
      for (size_t i = 0; i < in_overloads_len; ++i) {
        const cel_expr_Decl_FunctionDecl_Overload* in_overload =
            in_overloads_ptr[i];
        cel_StringView out_id;
        if (CEL_UNLIKELY(!cel_Arena_StrDup(
                arena, &out_id,
                cel_expr_Decl_FunctionDecl_Overload_overload_id(
                    in_overload)))) {
          cel_OutOfMemoryStatus(status);
          return cel_nullptr;
        }
        const cel_Type* out_result;
        if (cel_expr_Decl_FunctionDecl_Overload_has_result_type(
                in_overload)) {
          out_result = cel_Type_FromProto(
              cel_expr_Decl_FunctionDecl_Overload_result_type(
                  in_overload),
              arena, status);
        ...
        const cel_expr_Type* const* in_params_ptr =
            cel_expr_Decl_FunctionDecl_Overload_params(in_overload,
                                                              &in_params_len);
        ...
        for (size_t j = 0; j < in_params_len; ++j) {
          const cel_expr_Type* in_param = in_params_ptr[j];
          const cel_Type* out_param;
          if (in_param != cel_nullptr) {
            out_param = cel_Type_FromProto(in_param, arena, status);
        ...
        cel_FunctionOverloadDecl* out_overload = cel_FunctionOverloadDecl_New(
            out_id,
            cel_expr_Decl_FunctionDecl_Overload_is_instance_function(
                in_overload)
        ...
        if (CEL_UNLIKELY(!cel_Arena_StrDup(
                arena, &out_doc,
                cel_expr_Decl_FunctionDecl_Overload_doc(in_overload)))) {
          cel_OutOfMemoryStatus(status);
          return cel_nullptr;
        }
      ...
      if (CEL_UNLIKELY(!cel_Arena_StrDup(
              arena, &out_doc,
              cel_expr_Decl_FunctionDecl_doc(in_function)))) {
        cel_OutOfMemoryStatus(status);
        return cel_nullptr;
      }
      cel_Decl_SetDoc(cel_Decl_UpCast(out), out_doc);
    ...
    default:
      cel_InvalidArgumentStatusF(
          status, "cel: unexpected google.api.expr.Decl.decl_kind: %d",
          decl_kind);
      return cel_nullptr;
  }
}
