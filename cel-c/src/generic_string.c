...
bool _cel_GenericString_ShrinkToFitAllocator(CEL_NONNULL(_cel_GenericString*)
  ...
  if (str->large.is_large && str->large.cap > str->large.len) {
    ...
    CEL_NULLABLE(char*)
    data = cel_Allocator_Malloc(alloc, new_cap + 1, &actual_new_cap);
...
bool _cel_GenericString_AssignArena(CEL_NONNULL(_cel_GenericString*) str,
  ...
  if (str->large.is_large) {
    if (new_len > str->large.cap) {
      size_t new_cap = new_len;
      CEL_NULLABLE(char*)
      data = cel_Arena_Malloc(arena, new_cap + 1, cel_nullptr);
      if (CEL_UNLIKELY(data == cel_nullptr)) {
        _cel_GenericString_AnnotateLargeNew(str->large.data, str->large.len,
                                            str->large.cap);
        str->large.data[str->large.len] = '\0';
...
bool _cel_GenericString_StabilizeAllocator(CEL_NONNULL(_cel_GenericString*) str,
  ...
  size_t actual_new_cap;
  CEL_NULLABLE(char*)
  data = cel_Allocator_Malloc(alloc, len + 1, &actual_new_cap);
  if (CEL_UNLIKELY(data == cel_nullptr)) {
    return false;
  }
  --actual_new_cap;
...
bool _cel_GenericString_StabilizeArena(CEL_NONNULL(_cel_GenericString*) str,
  ...
  size_t actual_new_cap;
  CEL_NULLABLE(char*)
  data = cel_Arena_Malloc(arena, len + 1, &actual_new_cap);
  if (CEL_UNLIKELY(data == cel_nullptr)) {
    return false;
  }
  --actual_new_cap;
