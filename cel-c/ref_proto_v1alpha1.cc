...
extern "C" cel_Ref* cel_nullable cel_Ref_FromProtoV1Alpha1(
  ...
  if (!cel_Arena_StrDup(arena, &out_name,
                        google_api_expr_v1alpha1_Reference_name(in))) {
    cel_OutOfMemoryStatus(status);
    return cel_nullptr;
  }
  size_t in_overloads_len;
  const upb_StringView* in_overloads_ptr =
      google_api_expr_v1alpha1_Reference_overload_id(in, &in_overloads_len);
  if (in_overloads_len > 0 &&
      google_api_expr_v1alpha1_Reference_has_value(in)) {
  ...
  if (google_api_expr_v1alpha1_Reference_has_value(in)) {
    if (!cel_Constant_FromProtoV1Alpha1(
            cel_IdentRef_MutableValue(out),
            google_api_expr_v1alpha1_Reference_value(in), arena, status)) {
      return cel_nullptr;
    }
  }
  return cel_Ref_UpCast(out);
