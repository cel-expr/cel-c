  ...
  while (!upb_EpsCopyInputStream_IsDone(&state->stream, &ptr)) {
    ...
    switch (wire_type) {
      ...
      case kUpb_WireType_Delimited: {
        ...
        ptr = upb_EpsCopyInputStream_ReadStringAlwaysAlias(
            &state->stream, ptr, size, &sv);
...
static bool _cel_MessageEqualityState_HomoEquals(
  ...
  size_t lhs_iter = kUpb_Message_Begin;
  size_t rhs_iter = kUpb_Message_Begin;
  while (true) {
    bool lhs_has_next =
